...
impl TryFrom<UserRow> for User {
    ...
    fn try_from(row: UserRow) -> Result<Self, Self::Error> {
        ...
        Ok(User::from_persisted(
            ...
            email,
            phone,
            row.password_hash.map(PasswordHash::from),
            status,
            provider,
            row.created_at.into(),
            row.updated_at.into(),
...
fn user_active_model(user: &User) -> ActiveModel {
    ActiveModel {
        ...
        phone: Set(user.phone().map(|phone| phone.to_string())),
        password_hash: Set(user.password_hash().map(|hash| hash.as_ref().to_owned())),
        ...
        provider: Set(user.provider().map(|provider| match provider {
...
mod tests {
    ...
    fn sample_user() -> User {
        User::from_persisted(
            ...
            Some(Phone::new("+380501234567").expect("valid phone")),
            Some(PasswordHash::from("hashed-password".to_string())),
    ...
    fn maps_user_row_into_domain_user() {
        ...
        assert_eq!(user.phone().map(Phone::as_str), row.phone.as_deref());
